...
#include <ctime>
#include <sstream>
#include "../libquickmath/arithmetic.h"
        ...
        for (auto& p : _players) {
            ...
            if (q.op == Op::DIV) {
                ...
                if (space != string::npos) {
                    ...
                    // Concatenated format: try every split point (e.g. "14" → 1|4)
                    if (isNumber(p.answer) && !p.answer.empty() && p.answer[0] != '-') {
                        for (size_t split = 1; split < p.answer.size(); split++) {
                            int aq = stoi(p.answer.substr(0, split));
                            int ar = stoi(p.answer.substr(split));
                            if (aq == q.correct_div_q && ar == q.correct_div_r) {
                                correct = true;
                                break;
                            }
                        }
