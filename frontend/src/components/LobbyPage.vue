...
const { gamePhase, lobbyState, connect: wsConnect, disconnect: wsDisconnect, startGame } = useGameWs()
...
async function pollStatus() {
  ...
  try {
    const res = await fetch('/api/room/status?room_id=' + roomId.value, {
      headers: { 'Authorization': 'Bearer ' + token.value }
    })
    const data = await res.json()
    if (data.error) { console.warn('[Lobby] poll error:', data.error); return }
    _updatePlayers(data)
  } catch (e) {
...
// 实时大厅更新 (shíshí dàtīng gēngxīn — real-time lobby updates via WebSocket)
watch(lobbyState, (state) => {
  if (state.roomId === roomId.value && state.players.length > 0) {
    roomCode.value = state.code || roomCode.value
    players.value = state.players.map(p => ({
      ...p,
      me: p.userId === userId.value,
  ...
  <div class="card">
    ...
    <div v-if="!roomId">
      ...
      <div v-else class="tab-content">
        ...
        <button type="button" class="btn btn-primary" @click="joinRoom" :disabled="busy || !roomCode.trim()" style="width:100%;margin-top:0.5rem">
